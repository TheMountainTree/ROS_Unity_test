...
class CentrlControllerSSVEPNode4(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            ...
            "/reasoner/images",
            descriptor=desc("reasoner -> SSVEP 图片输入话题。"),
        )
        self.declare_parameter(
            "reasoner_output_topic",
            "/reasoner/feedback",
            descriptor=desc("SSVEP -> reasoner 命令/历史回传话题。"),
        )
        self.declare_parameter(
            "mock_selected_index",
            -1,
        ...
        self.declare_parameter(
            "history_image_topic",
            "/history_image",
            descriptor=desc("发给 Unity 历史栏的图片话题。"),
        )
        self.declare_parameter(
            "history_image_width",
            100,
            descriptor=desc("发给 Unity 历史栏图片的固定宽度。"),
        )
        self.declare_parameter(
            "history_image_height",
            100,
            descriptor=desc("发给 Unity 历史栏图片的固定高度。"),
        )
        self.declare_parameter(
            "history_udp_target_ip",
            "127.0.0.1",
            descriptor=desc("历史删除命令的 Unity UDP IP。"),
        )
        self.declare_parameter(
            "history_udp_target_port",
            12001,
            descriptor=desc("历史删除命令的 Unity UDP 端口。"),
        )
    ...
    def _load_common_config(self) -> None:
        ...
        self.history_image_width = self._param_int("history_image_width")
        self.history_image_height = self._param_int("history_image_height")
    ...
    def _load_shared_comm_config(self) -> None:
        ...
        self.reasoner_mode_enabled = self._param_bool("reasoner_mode_enabled")
        self.history_udp_target_ip = self._param_str("history_udp_target_ip")
        self.history_udp_target_port = self._param_int("history_udp_target_port")
        if self.history_image_width <= 0 or self.history_image_height <= 0:
    ...
    def _init_mode_specific(self) -> None:
        ...
        if self.run_mode == "decode":
            ...
            if self.reasoner_mode_enabled:
                self.base_images = self._generate_placeholders(self.decode_num_images)
            else:
    ...
    def _handle_reasoner_selection(self, selection: int) -> None:
        ...
        if selection in (0, 1, 2, 4, 5, 6):
            ...
            self.state = "reasoner_wait_batch"
            self.get_logger().info(
    ...
    def _finalize_decode_trial(self) -> None:
        ...
        if self.reasoner_mode_enabled:
            self.state = "reasoner_wait_selection"
    ...
    def _on_timer_decode(self, now: float) -> None:
        ...
        if self.state == "reasoner_wait_batch":
            if not self.reasoner_handshake_complete:
                return
        ...
        if self.state == "reasoner_wait_selection":
