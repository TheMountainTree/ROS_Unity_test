...
- **history 与回退逻辑**：
  ...
  - `7`：通过 UDP `delete_last` 删除 Unity history 最后一项，并回到上一组 6 张重新选择
- **history 缩略图固定尺寸**：为了兼容 `HistoryManager.cs` + ScrollView 布局，history 图片在 ROS 端统一缩放为 `100x100`（可通过 `history_image_width` / `history_image_height` 调整），避免直接发送 `640x480` 导致 Content 间距异常。
