windows端使用`windows_com_publisher.py`进行转发，python环境请激活`eeg`。注意修改com口，连接triggerbox后进设备管理器看是哪个com口，然后修改代码中的配置。
ubuntu端使用`upd_sender_node.py`节点进行发送
可以通过triggerbox上usb指示灯的闪烁判断是否成功trigger,又或者在软件`NeuroNexus`中，可以在下方看到红色带有标记的label,也说明成功转发。同时Displayer右侧的event中也能看到id和annotation
