#!/usr/bin/env python3
"""
SSVEP Pipeline — Modular Components (with Sliding Window)
=========================================================
...
2. **SSVEPPretrainer**  — Train FBTRCA (eTRCA) and persist projection matrices.
   Supports sliding‑window search: slides a fixed‑duration window across the
   epoch to find the optimal position (highest leave‑one‑out accuracy), then
   trains the final model on that window only.
...
production ROS nodes (e.g. CentralControllerSSVEPNode4Test) — they depend only on
...
class SSVEPPretrainer:
    ...
    model is then trained on data from that optimal window only.
    ...
    # ------------------------------------------------------------------
    def _make_estimator(self) -> FBTRCA:
        return FBTRCA(
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "SSVEPPretrainer":
        """Fit the eTRCA model on training data (full epoch, no window search).
        ...
        self._optimal_window_samples = int(X.shape[2])
    ...
    def fit_with_window_search(
        ...
        position, then train the final eTRCA model on that window.

        The search slides a window of ``window_duration_s`` seconds across
        the time axis in steps of ``window_step_s`` seconds.  At each
        position a leave‑one‑out accuracy is computed.  The position with
        the highest accuracy is selected and the final model is trained on
        data extracted from that window.
        ...
        self._estimator = self._make_estimator()
        self._estimator.fit(X=X_optimal, y=y)

        if verbose:
            print(f"[WindowSearch] final model trained on "
    ...
    def save(self, filepath: str) -> None:
        """Persist the trained model (incl. projection matrices and window info)."""
...
class SSVEPDecoder:
    ...
    If the pretrainer was trained with sliding‑window search, the decoder
    ...
    def decode(self, X: np.ndarray) -> np.ndarray:
        ...
        If the model was trained with sliding‑window search, the optimal
...
class SSVEPEvaluator:
    ...
    def cross_validate(
        ...
        use_window_search : bool
            If True, use :meth:`SSVEPPretrainer.fit_with_window_search`
