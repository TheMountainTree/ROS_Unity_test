...
- `src/eeg_processing/eeg_processing/SSVEP_Communication_Node4_test.py`: Node4_test wiring entry for eTRCA training/decoding + reasoner flow; keep mode wiring here and put behavior in `_2_test` modules.
- `src/eeg_processing/eeg_processing/pretrain_2_test.py`: Node4_test pretrain/shared EEG module; includes fixed-length resample, robust channel normalization, automatic bad-channel suppression during training, and saves a decode sidecar profile (`<model>.channel_profile.json`).
- `src/eeg_processing/eeg_processing/decode_2_test.py`: Node4_test decode module; includes variable-length eTRCA window voting, decode bandpass+notch filtering, fixed/dynamic bad-channel suppression, and UI 8-slot mapping alignment for reasoner selection.
...
- Node4_test model sidecar convention: training may write `<model_path>.channel_profile.json` with robust channel scales and bad channel indices; decode should consume this sidecar for fixed bad-channel suppression in addition to dynamic per-epoch suppression.
- Node4_test decode filtering defaults are config-level (`ETRCADecoderConfig`): bandpass + notch list (commonly 50/100 Hz for mains/monitor artifacts); keep suppress/normalization toggles in config rather than hardcoding in module logic.
